...
and t.nrlg_sys_desc is not null and t.nrlg_smaint is not null
...
and t.nrlg_dept_route in (select s.nrlg_dept_route from TIS.TIS_NEW_ROADLOG s 
                            where s.nrlg_dept_route in (select corridor_code from sec_segments t inner join TIS.TIS_NEW_ROADLOG s
                            on s.nrlg_dept_route = t.corridor_code
                            where t.secfile_name = 'North'
                            group by t.corridor_code)
                            group by s.nrlg_dept_route)
